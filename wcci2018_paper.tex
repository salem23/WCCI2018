...
\begin{abstract}
	...
	Games offer a suitable testbed where new methodologies and algorithms can be tested in a near-real life environment. For example, in a card driving game, using transfer learning or other techniques results can be
	...
	game TORCS. The Genetic Algorithm applied 
	%Pablo: How many? Why different?
	% Antonio - It is just one GA
	improves the fuzzy systems to set an optimal target speed as well as
	the instantaneous steering angle during the race.
	Thus, the approach offer an automatic way to define the
	membership functions, instead of a manual or hill-climbing descent method.
	However, the main issue with this kind of algorithms is to define a proper fitness function that best delivers the obtained result, which is eventually %Pablo: "delivers the obtained result"? Better "models the desired result"
	to win as many races as possible. In this paper we define two different evaluation functions, and prove that fine-tuning the controller via evolutionary algorithms 
	%Pablo: Algorithm or algorithms? Are the parameters tuned?
	% Antonio - just one, but this is the general term. No parameter tuning has been done
	robustly finds good results and that, in many cases, they are able to play very competitively against other published results, 
	%Pablo: our previous work?
	% Antonio - also other drivers for the SotA
	with a more relying approach that needs very few parameters to tune.
	The optimized fuzzy-controllers (one per fitness) yield a very good performance, mainly in tracks that have many turning points, which are, in turn, the most
	difficult for any autonomous agent. 
	Experimental results show that the enhanced controllers are very competitive with respect to the embedded TORCS drivers, and much more efficient in driving than the original fuzzy-controller.
	% comment new results Pablo: the objective should still be clarified, I still don't know what has been done in this paper by only reading the abstract
\end{abstract}
\begin{IEEEkeywords}
...
design and manufacture. % Maybe some review with popularity? - JJ Pablo: I don't understand the concept "enter the arena". Too dramatic :)
% Mohammed : I agree
Automotive industry
...
% Antonio - TORCS is not a game, it's a simulator. ;)
%Mohammed : one could drive a car  with joystick or keyboard in TORCS
can be placed in that context, with solutions obtained there having utility beyond the game itself. 
%This explains the popularity of car racing games challenges, which are
%usually performed using game simulators. 
...
challenges. 
% Antonio - actually there are no more competitions since many years ago. :_D
%Mohammed : put had been used in place of has been used ,,, no?
...
% Added back. It's essential, and we have space - JJ
...
in a practice race together. They have been run %Pablo: do not use future, if you are using past tense stick with it (past perfect can also be used)
...
% Say something also about being really better, or not, to include the topspeed in the fitness, and if maybe other fitness functions (without including the damage, for instance) should be possible.
% Antonio - written
...
% And paralellization so that speed of evolution can be improved.
% Antonio - written
